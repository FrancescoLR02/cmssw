...
# Firmware-style settings. NOT used by the emulator any more - kept because the skimmer and
# the flat-table converter take it as algoSettings for their pt/eta/quality conversions.
...
# The one the emulator actually runs. Iterative = True -> IterativeChain emits the second pass
# (slow) track, so there is a single track collection and it is the slow hypothesis.
bmtfKalmanTrackingOfflineSettingsSlow = cms.PSet(
  verbose = cms.bool(False),  #
  lutFile = cms.string("L1Trigger/L1TMuon/data/bmtf_luts/kalmanLUTs_v302.root"),
  initialK = cms.vdouble(-1.196,-1.581,-2.133,-2.263),
  initialK2 = cms.vdouble(-3.26e-4,-7.165e-4,2.305e-3,-5.63e-3),
...
run3_2024_L1T.toModify(
  bmtfKalmanTrackingOfflineSettingsSlow,
  useNewQualityCalculation = cms.bool(True),
)

process.kbmtfConvert = cms.EDProducer("convertToL1MuKBMTCombinedStub",
  src = cms.InputTag("FinalBxSelectorBMTFStub", "BMTFStub"),
  bxMin = cms.int32(1),
...
process.skimMuons = cms.EDProducer("SkimmerScoutingMuonCollection",
  gmtSrc = cms.InputTag("FinalBxSelectorMuon", "Muon"),
...
process.kbmtfEmulation = cms.EDProducer("L1TMuonBarrelScoutingKalmanTrackProducer",
  ...
  gmtSrc = cms.InputTag("FinalBxSelectorMuon", "Muon"),
  metSrc = cms.InputTag("FinalBxSelectorBxSums", "EtSum"),
